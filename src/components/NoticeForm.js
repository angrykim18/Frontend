...
function NoticeForm() {
    ...
    useEffect(() => {
        const fetchUserGroups = async () => {
            try {
                const response = await axios.get(`${process.env.REACT_APP_API_URL}/api/user-groups`);
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        const apiEndpoint = isEditMode ? `${process.env.REACT_APP_API_URL}/api/notices/${id}` : `${process.env.REACT_APP_API_URL}/api/notices`;
