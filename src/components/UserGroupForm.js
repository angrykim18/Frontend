...
function UserGroupForm() {
  ...
  useEffect(() => {
    if (isEditMode) {
      const fetchGroup = async () => {
        try {
          const response = await axios.get(`${process.env.REACT_APP_API_URL}/api/user-groups/${id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    const apiEndpoint = isEditMode ? `${process.env.REACT_APP_API_URL}/api/user-groups/${id}` : `${process.env.REACT_APP_API_URL}/api/user-groups`;
