...
function LiveChannelForm() {
  ...
  useEffect(() => {
    if (isEditMode) {
      ...
      const fetchChannel = async () => {
        try {
          const response = await axios.get(`${process.env.REACT_APP_API_URL}/api/live-channels/${id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEditMode) {
        // 수정 모드: PUT 요청
        await axios.put(`${process.env.REACT_APP_API_URL}/api/live-channels/${id}`, channel);
        alert('채널이 성공적으로 수정되었습니다.');
      } else {
        // 추가 모드: POST 요청
        await axios.post(`${process.env.REACT_APP_API_URL}/api/live-channels`, channel);
