...
function AgencyManagement() {
  ...
  const fetchAgencies = async () => {
    try {
      const response = await axios.get(`${process.env.REACT_APP_API_URL}/api/agencies`);
  ...
  const handleDelete = async (agencyId) => {
    if (window.confirm(`정말로 이 대리점(ID: ${agencyId})을 삭제하시겠습니까?`)) {
      try {
        await axios.delete(`${process.env.REACT_APP_API_URL}/api/agencies/${agencyId}`);
        alert('대리점이 삭제되었습니다.');
        fetchAgencies();
      } catch (error) {
        alert('삭제에 실패했습니다.');
  ...
  const handleCharge = async (agencyId) => {
    ...
    if (amount && !isNaN(amount)) {
        try {
            await axios.post(`${process.env.REACT_APP_API_URL}/api/agencies/${agencyId}/charge`, { amount: parseInt(amount) });
