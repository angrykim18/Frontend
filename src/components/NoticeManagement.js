...
function NoticeManagement() {
    ...
    const handleDelete = async (noticeId) => {
        if (window.confirm(`정말로 이 공지(ID: ${noticeId})를 삭제하시겠습니까?`)) {
            try {
                await axios.delete(`${process.env.REACT_APP_API_URL}/api/notices/${noticeId}`);
                alert('공지사항이 삭제되었습니다.');
                fetchNotices();
            } catch (error) {
                alert('삭제에 실패했습니다.');
            ...
            <table className="user-table">
                ...
                <tbody>
                    {notices.map(notice => (
                        <tr key={notice.id}>
                            <td>{notice.id}</td>
                            {/* ✅ [추가] 공지 타입(noticeType)을 표시하는 열을 추가합니다. */}
                            <td>
                                {notice.noticeType === '긴급' ? 
                                    <span style={{ color: 'red', fontWeight: 'bold' }}>{notice.noticeType}</span> : 
                                    notice.noticeType}
                            </td>
                            <td style={{ maxWidth: '400px', whiteSpace: 'pre-wrap' }}>{notice.content}</td>
