...
function ServerForm() {
    ...
    useEffect(() => {
        if (isEditMode) {
            const fetchServer = async () => {
                try {
                    const response = await axios.get(`${process.env.REACT_APP_API_URL}/api/servers/${id}`);
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        const apiEndpoint = isEditMode ? `${process.env.REACT_APP_API_URL}/api/servers/${id}` : `${process.env.REACT_APP_API_URL}/api/servers`;
